...
/**
 * @brief DisplayData::OnSave
 * 将导出的数据保存到本地。
 * 没有对写入操作进行监控，可能发生写入错误。
 */
void DisplayData::OnSave(){
    ...
    if(ui->lineEdit->text() == ""){
        if(!(saveExcel.saveAs(ui->lineEdit->placeholderText() + ".xlsx"))){
            QMessageBox::warning(this, tr("警告"), tr("文件保存失败"));
        }
    }else{
        if(!(saveExcel.saveAs(ui->lineEdit->text() + ".xlsx"))){
            QMessageBox::warning(this, tr("警告"), tr("文件保存失败"));
        }
    }
}
